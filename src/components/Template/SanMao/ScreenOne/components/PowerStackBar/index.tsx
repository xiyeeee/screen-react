...
const PowerStackBar: React.FC<Props> = (props) => {
  ...
  // 🎯 数据分组逻辑：将数据按月份分组
  const groupDataByMonths = (capacityData) => {
  ...
  // 🎯 将嵌套数组格式转换成堆叠柱状图所需格式
  const transformDataToStackBarData = (capacityData) => {
    if (!capacityData || !Array.isArray(capacityData)) {
      return {
    ...
    // 提取所有时间点
    const categories = capacityData.map((item) => item.timePoint);
    ...
    const series = loomNames.map((loomName, loomIndex) => ({
      name: loomName,
      data: capacityData.map((timeItem, timeIndex) => {
        const production = timeItem.productionByCapacityVOS.find(
          (p) => p.loomName === loomName
        );
  ...
  const currentGroupData = getGroupedData();
        ...
        params.forEach((param) => {
          result += `<div style="margin-bottom:3px;">
            <span style="display:inline-block;width:10px;height:10px;background:${
              param.color.colorStops
                ? param.color.colorStops[0].color
                : param.color
            };margin-right:8px;"></span>
            <span style="color:#fff;">${param.seriesName}: ${
            param.value
          }%</span>
          </div>`;
        });
